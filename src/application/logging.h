...
#define SNUFF_BREAK assert(false)

#define SNUFF_ASSERT(msg) std::string message = ##msg; std::string result = "\n\nSnuffbox assertion!\n----------------------\n" + message + "\n"; SNUFF_LOG_FATAL(result); while(true){ SNUFF_BREAK; }
#define SNUFF_XASSERT(expr, msg) if (!(expr)){ SNUFF_ASSERT(msg) }
#define SNUFF_ASSERT_NOTNULL(ptr) SNUFF_XASSERT(ptr != nullptr, "Attempt to get a null pointer!")
